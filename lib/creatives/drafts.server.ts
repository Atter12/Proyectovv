...
export async function approveCreativeDraft(input: {
  ...
  try {
    let advertiserId = draft.external_advertiser_id?.trim() || "";
    if (!advertiserId && draft.ad_account_id) {
      const { data: acc } = await admin
        .from("ad_accounts")
        .select("external_account_id")
        .eq("id", draft.ad_account_id)
        .maybeSingle<{ external_account_id: string | null }>();
      advertiserId = acc?.external_account_id?.trim() || "";
    }
    if (!advertiserId) {
      throw new Error("Falta advertiser_id de TikTok en el borrador.");
    }
