...
async function findIntentsByOperationCode(
  ...
  const { data, error } = await admin
    ...
    // Solo recargas del bot: la recarga manual se aprueba por gerente.
    .contains("metadata", { voucher_operation_code: operationCode, source: RECHARGE_BOT_SOURCE });
...
async function findCandidateIntents(
  ...
  // Cruzamos solo contra recargas abiertas por el bot del chat. Se guardan como
  // pago manual en soles para que, si el bot no puede validar, caigan en la cola
  // del gerente; pero la recarga manual del panel nunca se cruza aquí.
  //
  ...
  const { data, error } = await admin
    ...
    .eq("provider", "manual")
    // Solo recargas del bot. Una recarga manual en soles con el mismo monto
    // NO se toca: esa la aprueba un gerente, siempre.
