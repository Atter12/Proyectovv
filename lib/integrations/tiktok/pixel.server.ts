  ...
  if (!response.ok || (json.code != null && json.code !== 0)) {
    ...
    const hint =
      json.code === 40001 || /permission/i.test(baseMsg)
        ? " Tip: si 'Ver píxeles' falla pero el token es nuevo, prueba 'Crear píxel' (a veces TikTok deja crear/actualizar y no listar). Si crear también falla, en BC → Members → Manage permissions → Pixels."
        : "";
