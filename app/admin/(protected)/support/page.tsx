          ...
          <Table>
            ...
            <tbody className="divide-y divide-[var(--admin-table-divider)]">
              {tickets.map(({ row, organization, requester, assignee }) => (
                <tr key={row.id}>
                  <Td><Link href={`/admin/support/${row.id}`} className="font-semibold text-[var(--admin-text)] hover:text-[var(--admin-accent)]">{row.subject}</Link><p className="text-xs text-[var(--admin-text-muted)]">{formatDateTime(row.updated_at ?? row.created_at)}</p></Td>
                  <Td>{organization?.name ?? "—"}<p className="text-xs text-[var(--admin-text-muted)]">{row.category ?? "general"}</p></Td>
                  <Td>{requester?.email ?? "—"}<p className="text-xs text-[var(--admin-text-muted)]">Asignado: {assignee?.email ?? "sin asignar"}</p></Td>
