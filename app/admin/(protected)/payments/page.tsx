            ...
            <tbody className="divide-y divide-slate-100">
              {payments.map(({ row, organization, actor, proof }) => (
                <tr key={row.id}>
                  ...
                  <Td>{organization?.name ?? "—"}<p className="text-xs text-slate-400">{actor?.email ?? "—"}</p></Td>
                  <Td>{proof ? <span className="font-bold text-emerald-700">{proof.fileName ?? "Adjunto"}</span> : <span className="text-slate-400">Sin voucher</span>}<p className="text-xs text-slate-400">{proof?.uploadedAt ? formatDateTime(proof.uploadedAt) : ""}</p></Td>
