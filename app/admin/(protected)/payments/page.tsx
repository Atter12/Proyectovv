      ...
      <Card className="p-5">
        <form className="mb-5 grid gap-3 md:grid-cols-[1fr_13rem_auto]">
          <Input
            name="q"
            defaultValue={q}
            placeholder="Buscar por pago, cliente, referencia o correo…"
          />
        ...
        <TableWrap>
          <Table>
            ...
            <tbody className="divide-y divide-[var(--admin-table-divider)]">
              {payments.length === 0 ? (
                <tr>
                  <Td colSpan={7} className="py-10 text-center text-sm text-[var(--admin-text-muted)]">
                ...
                <tr key={row.id}>
                  <Td>
                    <Link
                      ...
                      className="font-semibold text-[var(--admin-text)] hover:text-[var(--admin-accent)]"
                    >
                      {row.id.slice(0, 8)}
                    </Link>
                  ...
                  <Td className="text-sm font-medium text-[var(--admin-text)]">
                    {row.provider === "crypto" ? "Cripto" : "Manual"}
                  </Td>
                  <Td>
                    {organization?.name ?? "—"}
                    <p className="text-xs text-[var(--admin-text-muted)]">
                  ...
                  <Td>
                      ...
                      <div className="space-y-1">
                        ...
                        {proof.signedUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="block text-xs font-semibold text-[var(--admin-accent)] hover:underline"
                          >
                            Ver preview
                          </a>
                        ) : null}
                    ...
                    <p className="text-xs text-[var(--admin-text-muted)]">
                      {proof?.uploadedAt ? formatDateTime(proof.uploadedAt) : ""}
                    </p>
                  </Td>
                      ...
                      {row.status !== "succeeded" && row.status !== "failed" ? (
                        <form action={rejectManualPaymentAction}>
                          ...
                          <input
                            type="hidden"
                            name="reason"
                            value="Voucher rechazado desde listado administrativo."
                          />
