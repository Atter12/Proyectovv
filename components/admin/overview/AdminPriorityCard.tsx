        ...
        <div className="grid min-h-0 flex-1 gap-5 max-lg:grid-cols-1 lg:grid-cols-[minmax(0,1.15fr)_auto_minmax(11.5rem,12.5rem)] lg:items-center lg:gap-6">
          <div className="min-w-0 space-y-4">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-1.5 rounded-full bg-[var(--admin-accent-soft)] px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.06em] text-[var(--admin-accent)]">
                <span className="h-1.5 w-1.5 rounded-full bg-[var(--admin-accent)]" aria-hidden />
                Prioridad del día
              </div>
              <h2 className="text-lg font-semibold tracking-tight text-[var(--admin-text)]">{priorityTitle(priorityTotal)}</h2>
              <p className="text-sm text-[var(--admin-text-muted)]">Cola financiera, soporte y webhooks.</p>
              <p className="text-xs text-[var(--admin-text-muted)]">{priorityOperationalHint(counts, priorityTotal)}</p>
            </div>
            <PriorityActions />
          </div>
