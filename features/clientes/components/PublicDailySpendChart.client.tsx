      ...
      {selected ? (
        <>
          <p id={`${id}-instructions`} className="sr-only">
            Importes diarios en dólares. Selecciona una barra o una fecha para ver los importes exactos.
          ...
          <div className="mt-3 grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-2">
            <button type="button" aria-label="Día anterior" disabled={selectedIndex === 0} onClick={() => selectDay(selectedIndex - 1)} className="flex h-11 w-11 items-center justify-center rounded-lg border border-[var(--admin-border)] hover:bg-[var(--admin-surface-hover)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--admin-accent)] disabled:cursor-not-allowed disabled:opacity-40">
            ...
            <button type="button" aria-label="Día siguiente" disabled={selectedIndex === series.length - 1} onClick={() => selectDay(selectedIndex + 1)} className="col-start-3 row-start-1 flex h-11 w-11 items-center justify-center rounded-lg border border-[var(--admin-border)] hover:bg-[var(--admin-surface-hover)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--admin-accent)] disabled:cursor-not-allowed disabled:opacity-40">
          ...
          <div aria-live="polite" aria-atomic="true" className="mt-3 grid min-w-0 grid-cols-2 gap-4">
            <p className="sr-only">Importes del {dateLabel(selected.key)}</p>
            <div className="min-w-0">
              <p className="text-xs text-[var(--admin-text-muted)]">Anuncios + comisión</p>
              <p className={`mt-1 font-semibold tabular-nums text-[var(--admin-accent-hover)] [overflow-wrap:anywhere] ${afterCutoff ? "text-sm leading-7" : "text-lg leading-7"}`}>{afterCutoff ? "Por actualizar" : money.format(selected.cargo)}</p>
              <p className="mt-0.5 text-xs leading-5 text-[var(--admin-text-muted)] [overflow-wrap:anywhere]">{afterCutoff ? "Acumulado al corte" : "Acumulado"}: <span className="tabular-nums">{money.format(selected.cargoCum)}</span></p>
            </div>
            <div className="min-w-0">
              <p className="text-xs text-[var(--admin-text-muted)]">Pagos aplicados</p>
              <p className="mt-1 text-lg font-semibold leading-7 tabular-nums text-[var(--admin-badge-success-text)] [overflow-wrap:anywhere]">{money.format(selected.paid)}</p>
              <p className="mt-0.5 text-xs leading-5 text-[var(--admin-text-muted)] [overflow-wrap:anywhere]">Acumulado: <span className="tabular-nums">{money.format(selected.paidCum)}</span></p>
            </div>
          </div>
        </>
      ) : (
