...
export function AffiliateNotes({ notes }: AffiliateNotesProps) {
  return (
    <div className="rounded-2xl border border-[#4056ff]/10 bg-gradient-to-br from-[#4056ff]/5 to-white p-5 sm:p-6">
          ...
          <ul className="mt-3 space-y-2.5">
            {notes.map((note) => (
              <li key={note} className="flex gap-2.5 text-sm leading-relaxed text-[#64748b]">
                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[#4056ff]" />
                <span>{note}</span>
              </li>
            ))}
          </ul>
